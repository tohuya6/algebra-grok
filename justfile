# algebra-grok task runner.  `just` recipes wrap `uv run` so no venv activation is needed.
...
# install optional extras too (wandb logging, HF checkpoints, nnsight interp)
setup-all:
    uv sync --extra wandb --extra pretrained --extra interp
...
train-symbolic name="symbolic" steps=steps *extra="":
    uv run python experiments/train_fixed_p.py \
...
eval dir *extra="":
    uv run python experiments/eval.py {{dir}} --device {{device}} {{extra}}
